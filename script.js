...
function execute() {
	if (validate()) {			//if function validate() returns true
		____________;		//then call function parseXML()
	}
}

/* function validate() will validate form data */
function validate() {
	var fileName = ____________;							//obtain the XML filename from the text box
	...
	if(____________) {											//check if a filename is entered
		...
		} else if(____________) {  							//check if the filename is nba.xml
...
function parseXML() {
	...
	var teamNames = xmlDoc.____________;									//accessing the nodes named "TeamName"
	var locations = xmlDoc.____________;										//accessing the nodes named "Location"
	var starPlayers = xmlDoc.____________;									//accessing the nodes named "StarPlayer"
	var stadiums = xmlDoc.____________;										//accessing the nodes named "Stadium"
	...
	for(var i=0; i<teams.length; ++i) {
		...
		var location = ____________;													//retrieve the value of a node in the array locations
		var starPlayer = ____________;												//retrieve the value of a node in the array starPlayers
		var stadium = ____________;													//retrieve the value of a node in the array stadiums
	...
	____________; 		//close the <table> element
...
function init() {
	____________;		//link function execute() to the click event of the button
	____________;		//link function execute() to the submit event of the form
}
